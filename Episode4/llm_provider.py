#!/usr/bin/env python3
"""
llm_provider.py — a thin adapter so multiagent.py runs on EITHER Anthropic
(Claude) or OpenAI (GPT), picked automatically from whichever API key is set.
Only ONE key needs to be present in .env:
...
parsing) lives in this one file. multiagent.py never imports `anthropic` or
`openai` directly — it only calls chat(), append_assistant_turn(), and
append_tool_results(), and reads the normalized Reply. That's what makes
...
def _detect_provider() -> str:
    ...
    raise RuntimeError(
        ...
        "Episode4/.env — see README.md 'Choose a provider' for how to get "
        "either one."
    )
