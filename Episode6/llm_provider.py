#!/usr/bin/env python3
"""
llm_provider.py — a thin adapter so safe_agent.py runs on EITHER
...
parsing) lives in this one file. safe_agent.py never imports
...
(This file is identical in spirit to Episode4/Episode5's llm_provider.py —
copied here rather than shared/imported across episodes so each episode's
folder stays independently runnable on its own.)
"""
...
load_dotenv(override=True)  # this project's .env wins over any stale shell-exported vars
...
def _detect_provider() -> str:
    ...
    raise RuntimeError(
        ...
        "Episode6/.env — see README.md 'Choose a provider' for how to get "
        "either one."
    )
